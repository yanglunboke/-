...
public class Exercise implements Serializable {
    ...
    public void generateWithFormerType(int operationCount) {
        ...
        switch(currentType) {
            ...
            case SUB:
                ...
                break;
            case MUL:
                this.generateMulExercise(operationCount);
            case DIV:
                this.generateDivExercise(operationCount);
        }
    }
    public enum ExerciseType {
        ADD , SUB, ADDandSUB, MUL ,DIV
    }
    //重构结束
    ...
    public void generateMulExercise(int operationCount){
        //随机生成加法算式并加入数组
        setCurrentType(ExerciseType.MUL ); //设置题目类型

        answers.clear();
        operationList.clear();
        current = 0;
        OperationCount = operationCount;
        FileName= "Mul_exercise_"+operationCount+"_";
        BinaryOperation anOperation1;
        while(operationCount>0){
            do{anOperation1 = new MulOperation();}while(contains(anOperation1));
            operationList.add(anOperation1);//将anOperation 加入到Exercise中
            answers.add(new Answers());
            operationCount--;
        }
    ...
    public void generateDivExercise(int operationCount){
        //随机生成加法算式并加入数组
        setCurrentType(ExerciseType.DIV ); //设置题目类型

        answers.clear();
        operationList.clear();
        current = 0;
        OperationCount = operationCount;
        FileName= "Div_exercise_"+operationCount+"_";
        BinaryOperation anOperation1;
        while(operationCount>0){
            do{anOperation1 = new DivOperation();}while(contains(anOperation1));
            operationList.add(anOperation1);//将anOperation 加入到Exercise中
            answers.add(new Answers());
            operationCount--;
        }
